...
import { updateData, updateTaskOrderInFirestore } from "../../../../firebase/firestore/updateData"
...
const ProjectTasks = () => {
  ...
  const addTask = async (data) => {
    ...
    await updateData({
      ...
      updatedData: {
        [`tasks.${newId}`]: newTask,
        [`columns.${columnId}.taskIds`]: [...initialDataArray.columns[columnId].taskIds, newId]
      }
    }).finally(loadTasks())
  }
  ...
  const onDragEnd = async (result) => {
    ...
    if (start === finish) {
      ...
      const { updatedStartColumn, updatedFinishColumn } = moveTaskBetweenColumns(
        start,
        finish,
        source.index,
        destination.index,
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          {initialDataArray &&
            initialDataArray.columnOrder.map((columnId) => {
              ...
              const tasks = column.taskIds.map((taskId) => initialDataArray.tasks[taskId])
              return (
                  ...
                  <Droppable droppableId={column.id}>
                    {(provided) => (
                      <TaskList ref={provided.innerRef} {...provided.droppableProps}>
                        {tasks.filter(task => !!task).map((task, index) => {
                          const levelMap = {
                            low: {
                            ...
                            medium: {
                              ...
                              background: tasksVariable.levelBackgroundColorOrange
                            },
                            high: {
                              color: tasksVariable.levelColorBlue,
                              background: tasksVariable.levelBackgroundColorBlue
                            },
                            completed: {
                              color: tasksVariable.levelColorGreen,
                              background: tasksVariable.levelBackgroundColorGreen
                            }
                          }
                                ...
                                <ProjectsCard
                                  ...
                                  <ProjectHeaderContainer>
                                    ...
                                    <TaskMenu
                                      ...
                                      <IconTaskMenu src="icon/dots_icon.svg" alt="Menu" />
                                    </TaskMenu>
                                    <TaskPupUp $taskMenuIsOpen={taskMenuIsOpen} $taskId={task.id}>
                                      <TaskButton>
                        ...
                        {column.taskIds.length === 0 && (
                          <DropInfo style={{ textAlign: "center", color: "#aaa" }}>
                            Drop tasks here
                          </DropInfo>
                        )}
                      </TaskList>
